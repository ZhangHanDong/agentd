...
fn main() {
    ...
    probe("D: ident with embedded hyphen", r#"digraph m { "x" [k=a-b]; }"#);

    // E: a single '-' not followed by '>' (e.g. negative number value)
    probe("E: attr value -5 (leading minus)", r#"digraph m { "x" [k=-5]; }"#);

    // F: empty pre_tools token via leading comma -> tool_name on "" 
    probe("F: pre_tools with empty token ',,'", r#"digraph m {
    ...
    probe("G: trailing // comment to EOF", "digraph m { \"a\" [shape=Mdiamond]; \"b\" [shape=Msquare]; \"a\"->\"b\" // trailing");

    // H: unbalanced parens in pre_tools -> split_top_level_commas depth underflow
    probe("H: pre_tools unbalanced parens", r#"digraph m {
        "start" [shape=Mdiamond];
    ...
    probe("I: quoted node literally named subgraph", r#"digraph m {
        "start" [shape=Mdiamond];
    ...
    probe("J: bare value with dot e.g. handler=wait.human", r#"digraph m {
        "start" [shape=Mdiamond];
